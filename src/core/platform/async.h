...
bool finishTask(const std::string& key,
...
{
    constexpr bool kTakesToken = std::is_invocable_v<WorkFn, const CancelToken&>;
    if constexpr (kTakesToken) {
        if constexpr (std::is_void_v<
                          std::invoke_result_t<WorkFn, const CancelToken&>>) {
            work(token);
            Result<void> result;
            result.ok = true;
            return result;
...
bool start(std::string key, bool restart, WorkFn work, ThenFn then)
{
    ...
    constexpr bool kTakesToken = std::is_invocable_v<Work, const CancelToken&>;
    using Raw = typename WorkRawType<Work, kTakesToken>::Type;
    using Value = ResultValueType<Raw>;
